...
def parse_lesson_path(path: str) -> dict[str, str]:
    """Parse lesson path to extract part, chapter, lesson components.

    Expected format: lessons/part-{N}/chapter-{M}/lesson-{K}.md

    Args:
        path: Lesson file path

    Returns:
        dict: Parsed components {"part": "part-1", "chapter": "chapter-01", "lesson": "lesson-01"}
    ...
    Example:
        ```python
        components = parse_lesson_path("lessons/part-1/chapter-01/lesson-01.md")
        # {"part": "part-1", "chapter": "chapter-01", "lesson": "lesson-01"}
        ```
    """
    parts = path.split('/')

    if len(parts) < 4:
        raise ValueError(f"Invalid lesson path format: {path}")

    if parts[0] != 'lessons':
        raise ValueError(f"Lesson path must start with 'lessons/': {path}")

    part = parts[1]
    chapter = parts[2]
    lesson_file = parts[3]

    # Remove .md extension from lesson
    lesson = lesson_file.replace('.md', '')
